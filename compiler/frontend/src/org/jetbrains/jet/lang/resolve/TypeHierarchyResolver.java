...
import static org.jetbrains.jet.lang.resolve.BindingContext.PACKAGE_TO_FILES;
...
public class TypeHierarchyResolver {
    ...
    public void process(
...

        {
            ...
            forDeferredResolve.addAll(collectNamespacesAndClassifiers(owner, declarations));
    ...
    private Collection<JetDeclarationContainer> collectNamespacesAndClassifiers(
        ...
        ClassifierCollector collector = new ClassifierCollector(owner, forDeferredResolve);
    ...
    private class ClassifierCollector extends JetVisitorVoid {
        ...
        public void visitJetFile(JetFile file) {
            // TODO 1 redeclaration
            NamespaceDescriptorImpl namespaceDescriptor = createNamespaceDescriptorPathIfNeeded(
                    file, RedeclarationHandler.DO_NOTHING);
    ...
    // TODO 1 this is from NamespaceFactoryImpl
    @NotNull
    private NamespaceDescriptorImpl createNamespaceDescriptorPathIfNeeded(@NotNull JetFile file,
        ...
        for (JetSimpleNameExpression nameExpression : namespaceHeader.getParentNamespaceNames()) {
            FqName fqName = namespaceHeader.getParentFqName(nameExpression);
            PackageFragmentDescriptor fragment = packageFragmentProvider.getOrCreateFragment(fqName);

            trace.record(BindingContext.NAMESPACE_IS_SRC, fragment, true);
            trace.record(RESOLUTION_SCOPE, nameExpression, packageFragmentProvider.getModule().getPackage(fqName).getMemberScope());
        }
    ...
    private void storeBindingForFileAndExpression(@Nullable JetFile file,
        ...
        if (expression != null) {
            trace.record(REFERENCE_TARGET, expression, packageView);
        }
        ...
        if (file != null) {
            ...
            Collection<JetFile> files = trace.get(PACKAGE_TO_FILES, packageView.getFqName());
            if (files == null) {
                files = Sets.newIdentityHashSet();
            }
            files.add(file);
            trace.record(BindingContext.PACKAGE_TO_FILES, packageView.getFqName(), files);
        }
    }
